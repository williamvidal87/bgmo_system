...
@push('requesting-service-table-scripts')
<script type="text/javascript">
    document.addEventListener('DOMContentLoaded', function () {
        ...
        window.livewire.on('openRequestServiceModal', () => {
            $('#RequestServiceModal').modal('show');
        });
        
        window.livewire.on('closeRequestServiceModal', () => {
            $('#RequestServiceModal').modal('hide')
        });
        
        window.livewire.on('openRequestServiceView', () => {
            $('#RequestServiceViewModal').modal('show')
        });
        
        window.livewire.on('closeRequestServiceView', () => {
            $('#RequestServiceViewModal').modal('hide')
        });
        ...
        window.livewire.on('openSwalDelete', (RequestServiceID) => {
            swal("Are you sure you want to delete this Data?", {
                dangerMode: true,
                buttons: true,
            }).then(okay => {
                if (okay) {
                    window.Livewire.emit('DeleteData', RequestServiceID)
                }else{
